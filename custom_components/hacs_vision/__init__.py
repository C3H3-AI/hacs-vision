...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigType) -> bool:
    ...
    # Auto-import token from HACS on first run (via GitHubClient)
...
async def _auto_import_token(hass: HomeAssistant, shared_data) -> None:
    ...
    """On first startup, if Vision has no token, try to import from HACS via GitHubClient."""

    try:

        from .github_client import GitHubClient

        gh = GitHubClient(hass, data=shared_data)

        current = await gh.get_token()

        if current:

            return  # Already have a token, skip

        result = await gh.import_from_hacs()

        if result.get("ok"):

            _LOGGER.info("Auto-imported GitHub token from HACS (user: %s)", result.get("user", "?"))
...
def _register_services(hass: HomeAssistant, operator) -> None:
    ...
    async def handle_find_entity_refs(call: ServiceCall) -> None:
        ...
        try:
            ...
            lines = [f"🔍 Entity ref search: {entity_id}", f"{len(refs)} refs across {len({(r['source_type'], r['source_id']) for r in refs})} sources\n"]
            ...
            await hass.services.async_call(
                ...
                {"title": f"HACS Vision - Entity Reference Search", "message": "\n".join(lines)},
        ...
        try:
            ...
            if preview:
                ...
                msg = (
                    ...
                    f"{result['total_refs']} references, {result['affected_count']} sources affected\n\n"

                    f"Send `preview: false` to execute the replacement"
                ...
                msg = (
                    ...
                    f"✅ **Replace complete** {old_id} → {new_id}\n"

                    f"Updated {total} occurrence(s)\n"
                    ...
                    f"Reload status: Auto{'✅' if reload.get('automations') else '❌'} "

                    f"Script{'✅' if reload.get('scripts') else '❌'} "

                    f"Scene{'✅' if reload.get('scenes') else '❌'}"
            ...
            await hass.services.async_call(
                ...
                {"title": f"HACS Vision - Entity Reference Replacement", "message": msg},
