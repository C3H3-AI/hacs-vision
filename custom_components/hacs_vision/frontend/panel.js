  ...
  `;async connectedCallback(){super.connectedCallback(),this._boundDocClick=e=>{if(!this._showSearchHistory)return;const t=this.shadowRoot?.querySelector(".search");if(t){e.composedPath().includes(t)||(this._showSearchHistory=!1)}},document.addEventListener("click",this._boundDocClick,!0),await this._loadFavorites(),await this._load(),this.addEventListener("install",e=>this._handleInstall(e.detail.repo)),this.addEventListener("update",e=>this._handleUpdate(e.detail.repo)),this.addEventListener("uninstall",e=>this._handleUninstall(e.detail.repo)),this.addEventListener("redownload",e=>this._handleRedownload(e.detail.repo)),this.addEventListener("ignore",e=>this._handleIgnore(e.detail.repo)),this.addEventListener("configure",e=>this._handleConfigure(e.detail.repo)),this.addEventListener("add-integration",e=>this._handleAddIntegration(e.detail.repo)),this.addEventListener("star-changed",e=>{const{repo:t,starred:i}=e.detail;t?.full_name&&(this._starredMap={...this._starredMap,[t.full_name]:i})}),this.addEventListener("favorite",e=>{const{isFavorite:t,repo:i}=e.detail,o=i?.id||i?.full_name;o&&(t&&!this._favorites.includes(o)?this._favorites=[...this._favorites,o]:t||(this._favorites=this._favorites.filter(e=>e!==o))),this._syncFavoriteCount()})}disconnectedCallback(){super.disconnectedCallback(),this._boundDocClick&&document.removeEventListener("click",this._boundDocClick,!0)}willUpdate(e){if(e.has("presetFilter")){const t=this.presetFilter;if(void 0===e.get("presetFilter")&&""===t)return;this.presetFilter="",this.statusFilter=t,this.page=1,this._persistState(),this._load()}if(e.has("presetTag")&&this.presetTag){const e=this.presetTag;this.presetTag="",this._tagFilters.includes(e)?this._tagFilters=this._tagFilters.filter(t=>t!==e):this._tagFilters=[...this._tagFilters,e],this.page=1,this._load()}}async _loadFavorites(){try{const e=await ce.getFavorites();this._favorites=Array.isArray(e)?e:e.favorites||[]}catch(e){this._favorites=[]}}_syncFavoriteCount(){this.requestUpdate()}_persistState(){!function(e){try{localStorage.setItem(Ht,JSON.stringify(e))}catch{}}({search:this.search,category:this.category,statusFilter:this.statusFilter,sort:this.sort,sortDir:this.sortDir,page:this.page,groupBy:this.groupBy,pageSize:this.pageSize})}async _toggleStar(e){const t=e.full_name;if(!t)return;const i=this._starredMap?.[t];try{i?(await ce.unstarRepo(t),this._starredMap={...this._starredMap,[t]:!1},e.stars=Math.max(0,(e.stars||e.stargazers_count||0)-1)):(await ce.starRepo(t),this._starredMap={...this._starredMap,[t]:!0},e.stars=(e.stars||e.stargazers_count||0)+1),this.requestUpdate()}catch(e){It(`Star 失败: ${e.message}`,"error")}}async _batchLoadStarStatus(){const e=(this.repos||[]).filter(e=>e?.full_name&&void 0===this._starredMap?.[e.full_name]);if(0===e.length)return;const t=await Promise.allSettled(e.slice(0,30).map(e=>ce.checkStarred(e.full_name).then(t=>({name:e.full_name,starred:t?.starred})))),i={};for(const e of t)"fulfilled"===e.status&&e.value&&"boolean"==typeof e.value.starred&&(i[e.value.name]=e.value.starred);this._starredMap={...this._starredMap,...i}}async _handleInstall(e){const t=e.id||e.full_name;this._installingIds={...this._installingIds,[t]:!0};try{await ce.install(t,e.category),It(`${ge("installComplete")}: ${e.full_name||e.name}`,"success"),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0})),this._load(),this._showPostInstallPrompt(e)}catch(e){It(`${ge("installFailed")}: ${e.message}`,"error")}const i={...this._installingIds};delete i[t],this._installingIds=i}async _showPostInstallPrompt(e){const t="integration"===(e.category||"integration");await new Promise(e=>setTimeout(e,1500));const{ConfirmDialog:i}=await Promise.resolve().then(function(){return Nt});if(t){if(!await i.show(this,{message:`${e.manifest_name||e.name} ${ge("postInstallRestartMsg")}`,confirmText:ge("restartHA"),cancelText:ge("later"),danger:!0}))return;try{await ce.restartHA(),It(ge("haRestarting"),"info")}catch(e){It(`${ge("restartFailed")}: ${e.message}`,"error")}}else{It(ge("reloadingHA"),"info");try{const e=await ce.reloadHA();e.success?It(ge("reloadSuccess"),"success"):It(`${ge("coreReloadFailed")}: ${e.error}`,"error")}catch(e){It(`${ge("coreReloadFailed")}: ${e.message}`,"error")}}}async _handleUpdate(e){try{const t=await ce.update([e.id||e.full_name]);t?.success?.length>0?It(`${ge("updateComplete")}: ${e.full_name||e.name}`,"success"):It(`${ge("updateFailed")}: ${e.full_name||e.name}`,"error"),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0})),this._load(),this._showPostInstallPrompt(e)}catch(e){console.error("Update failed",e),It(`${ge("updateFailed")}: ${e.message}`,"error")}}async _handleRedownload(e){try{const t=await ce.redownload(e.id||e.full_name,e.category);t?.success?It(`${ge("redownload")}: ${e.full_name||e.name}`,"success"):It(`${ge("updateFailed")}: ${t?.error||e.full_name}`,"error"),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0})),this._load()}catch(e){It(`${ge("updateFailed")}: ${e.message}`,"error")}}async _handleIgnore(e){if(await Ut.show(this,{message:ge("confirmIgnore",{repo:e.full_name||e.name}),confirmText:ge("ignore"),danger:!1}))try{await ce.ignoreRepo(e.id||e.full_name),It(`${ge("ignore")}: ${e.full_name||e.name}`,"success"),this._load()}catch(e){It(`${ge("updateFailed")}: ${e.message}`,"error")}}async _handleUninstall(e){if(await Ut.show(this,{message:ge("confirmRemove",{repo:e.full_name||e.name}),confirmText:ge("remove"),danger:!0}))try{await ce.remove(e.id||e.full_name),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0})),this._load()}catch(e){console.error("Uninstall failed",e),It(`${ge("updateFailed")}: ${e.message}`,"error")}}_handleConfigure(e){const t=e.domain||(e.full_name||"").split("/")[1]||"";this.dispatchEvent(new CustomEvent("open-options-flow",{detail:{entryId:e.config_entry_id,domain:t},bubbles:!0,composed:!0}))}_handleAddIntegration(e){const t=e.domain||(e.full_name||"").split("/")[1]||"";this.dispatchEvent(new CustomEvent("open-flow",{detail:{domain:t},bubbles:!0,composed:!0}))}async _load(){this.loading=!0;try{const e=!(!this.search||!this.search.match(/github\.com\/([^/]+\/[^/\s?#]+)/i)&&!this.search.match(/^[a-zA-Z0-9_-]+\/[a-zA-Z0-9_.-]+$/));let t=this.search;if(e){const e=this.search.match(/github\.com\/([^/]+\/[^/\s?#]+)/i);e&&(t=e[1].replace(/\.git$/,""))}const i=1===this._tagFilters.length?this._tagFilters[0]:"",o=await ce.listRepositories({search:t,category:this.category,sort:this.sort,sortDir:this.sortDir,page:this.page,limit:this.limit,status:this.statusFilter,tag:i});this.repos=o.repositories||[],this.total=o.total||0,this.categoryCounts=o.category_counts||{},this.statusCounts=o.status_counts||{},this.tagCounts=o.tag_counts||{},this.tagCounts={...this.tagCounts,favorites:this._favorites.length},this._batchLoadStarStatus()}catch(e){console.error("Browse load error",e),this.repos=[],this.total=0}this.loading=!1}_onSearch(e){this._searchText=e.target.value,this._showSearchHistory=!1,clearTimeout(this._searchTimer),this._searchTimer=setTimeout(()=>{this.search=this._searchText,this.page=1,this._persistState(),this._load(),this._searchText&&function(e){if(!e||!e.trim())return;e=e.trim();let t=qt();t=t.filter(t=>t!==e),t.unshift(e),t.length>10&&(t=t.slice(0,10));try{localStorage.setItem(Vt,JSON.stringify(t))}catch{}}(this._searchText)},300)}_onSearchFocus(){const e=qt();e.length>0&&(this._searchHistory=e,this._showSearchHistory=!0)}_onSearchFocusOut(e){e.currentTarget.contains(e.relatedTarget)||(this._showSearchHistory=!1)}_onSearchHistoryClick(e){this._showSearchHistory=!1,this._searchText=e,this.search=e,this.page=1,this._persistState(),this._load()}_clearSearchHistory(e,t){e.stopPropagation(),function(e){let t=qt().filter(t=>t!==e);try{localStorage.setItem(Vt,JSON.stringify(t))}catch{}}(t),this._searchHistory=qt(),0===this._searchHistory.length&&(this._showSearchHistory=!1)}_clearSearch(){this._searchText="",this.search="",this.page=1,this._showSearchHistory=!1,this._persistState(),this._load()}_onStatusFilter(e){this.statusFilter=e,this.page=1,this._persistState(),this._load()}_onTypeFilter(e){this.category=e,this.page=1,this._persistState(),this._load()}_onTagFilter(e){this._tagFilters.includes(e)?this._tagFilters=this._tagFilters.filter(t=>t!==e):this._tagFilters=[...this._tagFilters,e],this.page=1,this._load()}_onSortColumn(e){this.sort===e?this.sortDir="desc"===this.sortDir?"asc":"desc":(this.sort=e,this.sortDir="name"===e?"asc":"desc"),this.page=1,this._persistState(),this._load()}_onGroupChange(e){this.groupBy=e.target.value,this._persistState()}_onViewModeChange(e){this.viewMode=e;try{localStorage.setItem(jt,e)}catch{}}_toggleGroup(e){this._collapsedGroups={...this._collapsedGroups,[e]:!this._collapsedGroups[e]}}_goPage(e){this.page=e,this._persistState(),this._load()}_onPageSizeChange(e){this.pageSize=parseInt(e.target.value,10),this.limit=this.pageSize,this.page=1,this._persistState(),this._load()}_refresh(){this.page=1,this._persistState(),this._load()}async _addRepo(){const e=this._parseRepoUrl(this._newRepoUrl);if(e){this._addRepoInstalling=!0;try{const t=await ce.addCustomRepo(e,this._newRepoCategory);t.success?(It(`${ge("addSuccess")}: ${e}`,"success"),this._newRepoUrl="",this._showAddRepo=!1,this._load(),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0}))):It(`${ge("addFailed")}: ${t.error}`,"error")}catch(e){It(`${ge("addFailed")}: ${e.message}`,"error")}this._addRepoInstalling=!1}else It(ge("invalidRepoUrl"),"error")}_parseRepoUrl(e){const t=e.match(/github\.com\/([^/]+\/[^/\s?#]+)/i);return t?t[1].replace(/\.git$/,""):/^[a-zA-Z0-9_-]+\/[a-zA-Z0-9_.-]+$/.test(e)?e:null}_isRepoUrl(e){return e?.includes("/")}get _browseFilteredSortedOrgRepos(){const e=(this._orgFilter||"").trim().toLowerCase();return e?[...this._orgRepos].filter(t=>t.full_name.toLowerCase().includes(e)).sort((t,i)=>{const o=t.full_name.toLowerCase(),r=i.full_name.toLowerCase();return o===e&&r!==e?-1:r===e&&o!==e?1:o.startsWith(e)&&!r.startsWith(e)?-1:r.startsWith(e)&&!o.startsWith(e)?1:o.length-r.length}):this._orgRepos}get _browseOrgFilteredCount(){return this._browseFilteredSortedOrgRepos.length}async _loadBrowseOrgRepos(){const e=this._newRepoUrl?.trim();if(!(!e||this._isRepoUrl(e)||e.length<3)){this._orgLoading=!0,this._orgRepos=[],this._selectedOrgRepos={},this._orgSyncResult="";try{const t=await ce.listOrgRepos(e);t?.repos&&(this._orgRepos=t.repos)}catch(e){if(404===e.status)return;It(`${ge("loadFailedSimple")}: ${e.message}`,"error")}this._orgLoading=!1}}_toggleBrowseOrgRepo(e){if(this._selectedOrgRepos[e]){const t={...this._selectedOrgRepos};delete t[e],this._selectedOrgRepos=t}else this._selectedOrgRepos={...this._selectedOrgRepos,[e]:!0}}_toggleSelectAllBrowseOrg(e){const t=this._browseFilteredSortedOrgRepos;if(e){const e={};t.forEach(t=>e[t.full_name]=!0),this._selectedOrgRepos=e}else this._selectedOrgRepos={}}async _syncSelectedBrowseOrg(){const e=Object.keys(this._selectedOrgRepos);if(0!==e.length){this._orgSyncing=!0;try{const t=await ce.syncStarred(e),i=t?.added||t?.success?.length||0,o=t?.failed?.length||0;this._orgSyncResult=o?`${ge("addedToCustomList")} (${i}), ${o} ${ge("failedSuffix")}`:`${ge("addedToCustomList")} (${i})`}catch(e){this._orgSyncResult=`${ge("loadFailedSimple")}: ${e.message}`}this._orgSyncing=!1}}_getRepoStatus(e){return e.pending_restart?"pending_restart":e.installed&&(e.has_update||e.installed_version&&e.latest_version&&e.installed_version!==e.latest_version)?"pending-upgrade":e.installed?"installed":"default"}_getStatusBadge(e){const t={installed:{label:ge("statusInstalled"),cls:"installed"},"pending-upgrade":{label:ge("statusPendingUpgrade"),cls:"pending-upgrade"},"pending-restart":{label:ge("statusPendingRestart"),cls:"pending-restart"},default:{label:ge("statusDefault"),cls:"default"}}[e]||{label:e,cls:"default"};return H`<span class="status-badge ${t.cls}">${t.label}</span>`}_getStatusLabel(e){return{installed:ge("statusInstalled"),not_installed:ge("statusDefault"),update_available:ge("statusPendingUpgrade"),pending_restart:ge("statusPendingRestart")}[e]||e}_applyFilters(e){return this._tagFilters.includes("favorites")?e.filter(e=>this._favorites.includes(e.id||e.full_name)):e}_getFiltered(){let e=this._applyFilters(this.repos);if(!this.search)return e;const t=this.search.toLowerCase();let i=null;const o=t.match(/github\.com\/([^/]+\/[^/\s?#]+)/i);return o&&(i=o[1].replace(/\.git$/,"").toLowerCase()),e.filter(e=>{const o=(e.full_name||"").toLowerCase(),r=(e.name||"").toLowerCase(),s=(e.description||"").toLowerCase(),a=(e.authors||[]).join(",").toLowerCase(),n=(e.manifest_name||e.repository_manifest?.name||"").toLowerCase();return!!(o.includes(t)||r.includes(t)||s.includes(t)||a.includes(t))||(!(!i||o!==i&&!o.includes(i))||!!n.includes(t))})}_groupRepos(e){if("none"===this.groupBy)return null;const t={};for(const i of e){let e;e="status"===this.groupBy?this._getRepoStatus(i):"type"===this.groupBy&&i.category||"other",t[e]||(t[e]=[]),t[e].push(i)}const i=["pending-restart","pending-upgrade","installed","default"],o=["integration","plugin","theme","template","other"],r=Object.keys(t);return"status"===this.groupBy?r.sort((e,t)=>i.indexOf(e)-i.indexOf(t)):"type"===this.groupBy&&r.sort((e,t)=>o.indexOf(e)-o.indexOf(t)),r.map(e=>({key:e,label:"status"===this.groupBy?this._getStatusLabel(e):this._getCategoryLabel(e),repos:t[e]}))}_getCategoryLabel(e){return{integration:ge("catIntegration"),plugin:ge("catPlugin"),theme:ge("catTheme"),template:ge("catTemplate"),other:e}[e]||e}_formatDate(e){if(!e)return"";try{const t=new Date(e),i=new Date-t,o=Math.floor(i/864e5);return 0===o?ge("today")||"今天":1===o?ge("yesterday")||"昨天":o<30?`${o}d`:o<365?`${Math.floor(o/30)}mo`:`${Math.floor(o/365)}y`}catch{return""}}_toggleSelect(e){this._selectedRepos.includes(e)?this._selectedRepos=this._selectedRepos.filter(t=>t!==e):this._selectedRepos=[...this._selectedRepos,e]}_isAllSelected(){const e=this._getFiltered();return e.length>0&&this._selectedRepos.length===e.length}_toggleSelectAll(){const e=this._getFiltered();this._isAllSelected()?this._selectedRepos=[]:this._selectedRepos=e.map(e=>e.full_name).filter(Boolean)}async _batchDo(e){if(0===this._selectedRepos.length)return;const t=this._selectedRepos.map(e=>{const t=this.repos.find(t=>t.full_name===e);return{repository:e,category:t?.category||"integration"}});if("remove"===e){const{ConfirmDialog:e}=await Promise.resolve().then(function(){return Nt});if(!await e.show(this,{message:ge("batchRemoveConfirm",{n:t.length}),confirmText:ge("batchRemove"),danger:!0}))return}try{let i;It(ge("batchInProgress"),"info"),i="install"===e?await ce.batchInstall(t):"update"===e?await ce.update(t.map(e=>e.repository)):await ce.batchRemove(t.map(e=>e.repository)),It(ge("batchComplete"),"success"),this._selectedRepos=[],this._load()}catch(t){It(`${e} failed: ${t.message}`,"error")}}async _restartHA(){const{ConfirmDialog:e}=await Promise.resolve().then(function(){return Nt});if(await e.show(this,{message:this.t?.("restartConfirm"),confirmText:this.t?.("restartHA"),danger:!0}))try{await ce.restartHA(),It(this.t?.("haRestarting"),"info")}catch(e){It(`${this.t?.("restartFailed")}: ${e.message}`,"error")}}_renderRepoList(e){return"list"===this.viewMode?H`<div class="list-view">${this._renderListTable(e)}</div>`:H`<div class="grid">${e.map(e=>H`
    ...
    `}render(){const e=Math.ceil(this.total/this.limit),t=this._getFiltered(),i=this._groupRepos(t);return H`
      ...
      <div class="controls">
        <div class="search" @focusout=${this._onSearchFocusOut}>
          ...
          <input type="text" autocomplete="off" placeholder="${ge("searchPlaceholder")}" .value=${this._searchText} @input=${this._onSearch} @focus=${this._onSearchFocus} />
    ...
    `}render(){const e=this._getFiltered();return H`
      ...
      `:H`
        ...
        `}
      `}
    `}}customElements.define("updates-view",Wt);const Yt="hacs_vision_mgmt_state";function Xt(e){try{localStorage.setItem(Yt,JSON.stringify(e))}catch{}}class Jt extends ae{static properties={customRepos:{type:Array},archivedRepos:{type:Array},ignoredRepos:{type:Array},renamedEntries:{type:Array},loading:{type:Boolean},_customRepoUrl:{type:String},_customRepoCategory:{type:String},_showAddCustom:{type:Boolean},_addingCustom:{type:Boolean},erLoading:{type:Boolean},iLoading:{type:Boolean},importing:{type:Boolean},exporting:{type:Boolean},_viewMode:{type:String},_collapsed:{type:Object},_renamedRefreshing:{type:Boolean},_customRepoSearch:{type:String},_customRepofilter:{type:String},_customRepoSort:{type:String},_statusFilter:{type:String},_typeFilter:{type:String},_selectedRepos:{type:Array,state:!0},_favorites:{type:Array,state:!0},_tagFilters:{type:Array,state:!0},_orgUrl:{type:String},_orgRepos:{type:Array,state:!0},_orgLoading:{type:Boolean,state:!0},_orgFilter:{type:String,state:!0},_selectedOrgRepos:{type:Object,state:!0},_orgSyncResult:{type:String,state:!0},_orgSyncing:{type:Boolean,state:!0}};constructor(){super();const e=function(){try{return JSON.parse(localStorage.getItem(Yt)||"{}")}catch{return{}}}();this.customRepos=[],this.archivedRepos=[],this.ignoredRepos=[],this.renamedEntries=[],this.loading=!1,this.exporting=!1,this.importing=!1,this._renamedRefreshing=!1,this._showAddCustom=!1,this._customRepoUrl="",this._customRepoCategory="integration",this._addingCustom=!1,this._viewMode="card",this._customRepoSearch="",this._customRepofilter=e.customRepofilter||"all",this._customRepoSort=e.customRepoSort||"name",this._statusFilter=e.statusFilter||"",this._typeFilter=e.typeFilter||"",this._selectedRepos=[],this._favorites=[],this._tagFilters=[],this._collapsed={customRepos:!1,archived:!1,ignored:!1,tools:!1},this._orgRepos=[],this._orgRepos=[],this._orgLoading=!1,this._orgFilter="",this._selectedOrgRepos={},this._orgSyncResult="",this._orgSyncing=!1}static styles=[me(),s`
    :host { display: block; touch-action: manipulation; background: var(--primary-background-color); }
    ...
    `}render(){const{archivedRepos:e,ignoredRepos:t,renamedEntries:i,customRepos:o,loading:r,_viewMode:s,_collapsed:a}=this;return H`
      ...
      <div class="filter-sort-row">
        <div class="fs-chips">
          ...
          ${i.length>0?H`<button class="filter-chip ${"renamed"===this._customRepofilter?"active":""}" @click=${()=>{this._customRepofilter="renamed",Xt({customRepofilter:this._customRepofilter,customRepoSort:this._customRepoSort,statusFilter:this._statusFilter,typeFilter:this._typeFilter})}}>${ge("renamedRepos")} (${i.length})</button>`:""}
