...
### v6.4.6 (2026-07-15) — 修复第三方仓库翻译失败
- **修复**: **翻译改用前端已加载原文** — 原先翻译时后端会二次向 GitHub 拉取 README 原文，但 hacs_vision 的 GitHub Token 对第三方仓库（如 `al-one/xiaomi_miot`）返回 404，导致「按中文没反映」。现改为直接复用前端已加载的原文 HTML，彻底规避 Token 权限问题，所有仓库均可正常翻译
- **优化**: **翻译超时与诊断** — 后端调用对话助手增加 150s 超时，前端 AbortController 160s 兜底；精简翻译 prompt 并埋点计时日志，长文档翻译更稳更快

### v6.4.5 (2026-07-16) — 翻译语言可配置
- **新增**: **翻译语言可自定义** — 设置页「基本设置」新增「README 翻译语言」多选，勾选后对应语言按钮才出现在仓库弹窗的「说明文档」语言条中（原文始终显示）；未勾选时默认显示中文 / 英文 / 德文
- **新增**: **可选语言扩展** — 翻译语言由原本的中/英/德三种扩展至 **中文 / 英文 / 德文 / 日文 / 韩文** 五种，满足多语言查看需求
